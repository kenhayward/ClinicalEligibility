...
- `RunGate` (singleton) holds at most one in-flight `(runId, CancellationTokenSource)` pair — a second trigger arriving while busy returns `409 Conflict` with the current run id. **In-process only**: it is a CLR lock, so it cannot see the CLI (`elig run`) driving the same orchestrator against the same database.
- `PostgresRunLock` covers that gap: a **session-level Postgres advisory lock** taken by `BatchRunner` and by the CLI's `run` command immediately before the orchestrator starts, so a web batch and a CLI batch cannot run concurrently (both would anti-join the attempted set independently, select overlapping trials, and compete for the same model-server slots). Deliberately layered *under* `RunGate` rather than replacing it: the dashboard keeps its fast in-process 409 and no request thread touches the database. A web run refused by the lock logs a warning and broadcasts `OnBatchCancelledAsync`; a CLI run refused by it exits `2`. Session-scoped means process death releases it automatically - nothing to wedge. A dropped connection also releases it silently; the exposure is bounded to today's unguarded behaviour, and `Keepalive=60` on the output connection string removes it.
